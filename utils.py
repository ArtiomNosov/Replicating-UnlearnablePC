...
def compute_cat_iou(pred,target,iou_tabel):
    ...
    for j in range(pred.size(0)):
        ...
        for cat in np.unique(batch_target):
            ...
            iou_tabel[cat,0] += iou
            iou_tabel[cat,1] += 1
            iou_params.append(iou)
    return iou_tabel,iou_params
...
def test_partseg(model, loader, catdict, num_classes = 50,forpointnet2=False):
    ...
    iou_params = []
    metrics = defaultdict(lambda:params())
    hist_acc = []
    # mean_correct = []
    ...
    metrics['inctance_avg_iou'] = np.mean(iou_params)
...
def test_semseg(model, loader, catdict, num_classes = 13, pointnet2=False):
    ...
    for batch_id, (points, target) in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        if pointnet2:
            ...
            pred, _ = model(points)
        # print(pred.size())
        iou_tabel, iou_params = compute_cat_iou(pred,target,iou_tabel)
        # shape_ious += compute_overall_iou(pred, target, num_classes)
...
def get_list(mode, args):
    ...
    if mode == 'rot':
        ...
        x_params, y_params = [random.uniform(0, args.slight_range) for _ in range(Avg_num)], [random.uniform(0, args.slight_range) for _ in range(Avg_num)]
        z_params = [random.uniform(0, args.main_range) for _ in range(Avg_num)]

        for i in range(Avg_num):
            for j in range(Avg_num):
                for k in range(Avg_num):
                    params.append([x_params[i], z_params[j], y_params[k]])

        params = random.sample(params, args.NUM_CLASSES)

    elif mode == 'scale':
        params = [random.uniform(args.sca_min, args.sca_max) for _ in range(args.NUM_CLASSES)]

    elif mode == 'shear':
        params = [random.uniform(0, 0.4) for _ in range(args.NUM_CLASSES)]

    elif mode == 'twist':
        params = [random.uniform(0, 20) for _ in range(args.NUM_CLASSES)]

    elif mode == 'taper':
        params = [random.uniform(0, 50) for _ in range(args.NUM_CLASSES)]

    elif mode == 'translation':
        params = [[random.uniform(0, 0.3), random.uniform(0, 0.3), random.uniform(0, 0.3)] for _ in range(args.NUM_CLASSES)]
    return params 
...
def class_wise_reverse_transformation(point_cloud, mode, params, label):
    if torch.is_tensor(point_cloud):
        point_cloud = point_cloud.clone().detach().cpu().numpy()
    if mode == 'rot':
        x = np.pi / 180. * params[label][0]
        y = np.pi / 180. * params[label][1]
        z = np.pi / 180. * params[label][2]
        ...
        scaling_factor = params[label]
        reverse_sf = 1/params[label]
        matrix = np.array([ [reverse_sf, 0, 0], [0, reverse_sf, 0], [0, 0, reverse_sf] ])
        transformed_pc = np.matmul(point_cloud, matrix).astype('float32')
        return transformed_pc

    elif mode == 'shear':
        a = params[int(label)][0] if isinstance(params[int(label)], (list, tuple)) else params[int(label)]
        b = params[int(label)][1] if isinstance(params[int(label)], (list, tuple)) else params[int(label)]
        matrix = np.array([[1, 0, 0],[0, 1, 0],[-a, -b, 1]])
        transformed_pc = np.matmul(point_cloud, matrix).astype('float32')
        return transformed_pc   
 
    elif mode == 'twist':
        angle = np.pi / 180 * params[label]
...
def class_wise_transformation(point_cloud, mode, params, label):
    if torch.is_tensor(point_cloud):
        point_cloud = point_cloud.clone().detach().cpu().numpy()
    if mode == 'rot':
        x = np.pi / 180. * params[label][0]
        ...
        matrix = np.array([[params[label], 0, 0], [0, params[label], 0], [0, 0, params[label]]])
        transformed_pc = np.matmul(point_cloud, matrix).astype('float32')
        return transformed_pc
        ...
        factor = np.pi / 180. * params[label] * point_cloud[:, 2] + 1
        ...
        point_cloud[:, 0] += params[label][0]
        point_cloud[:, 1] += params[label][1]
        point_cloud[:, 2] += params[label][2]
        return point_cloud
...
class SORDefense(nn.Module):
    ...
    def process_data(self, pc, npoint=None):
        ...
        Args:
            pc (torch.FloatTensor): params input, [(N_i, 3)] from SOR.
